...
export class DatabaseStorage implements IStorage {
  ...
  async upsertUser(userData: UpsertUser): Promise<User> {
    ...
    if (userData.email) {
      ...
      if (existingUser) {
        // Update existing user with new data (like Replit auth data)
        const [user] = await db
          .update(users)
          .set({
            ...userData,
    ...
    // If no existing user found, insert new one
    const [user] = await db
      .insert(users)
      .values(userData)
      .onConflictDoUpdate({
  ...
  async getAllUserComprehensiveInteractions(userId: string): Promise<ComprehensiveInteraction[]> {
    return await db
      .select()
      .from(comprehensiveInteractions)
